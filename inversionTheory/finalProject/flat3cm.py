...
def main(args):
  ...
  horizonExtraction(k1,k2,k3,azimuth,elevation,h1,pg1,"surf1")
  k1,k2,k3 = 85,419,5 
  azimuth,elevation=100,40
  horizonExtraction(k1,k2,k3,azimuth,elevation,h2,pg2,"surf2")
...
def flatten():
  ...
  fl.setSmoothings(2.0,4.0)
  fl.setWeight1(0.08)
  fl.setScale(0.00001)
  fm = fl.getMappingsFromSlopes(s1,s2,s3,p2,p3,wp,ws,k4,k1,k2,k3)
  gc = fm.flatten(f)
  writeImage("gmfc",gc)
  s = fm.getShiftsS()
  writeImage("gmsc",s)
...
def slices():
  ...
  plot3f(k1,k2,k3,gm,cbar,label1,gmap=gray,cint=1.0,gmin=fmin,gmax=fmax,png="seismic")
  label1,cbar = "RGT","Amplitude"
  k1 = h1
  plot3f(k1,k2,k3,fc,cbar,label1,gmap=gray,cint=1.0,gmin=fmin,gmax=fmax,png="fl1")
  k1 = h2
  plot3f(k1,k2,k3,fc,cbar,label1,gmap=gray,cint=1.0,gmin=fmin,gmax=fmax,png="fl2")
  label1,cbar = "Time (s)","RGT"
  k1 = h2
  plot3f(k1,k2,k3,fu,cbar,label1,gmap=jet,cint=0.2,gmin=rmin,gmax=rmax,png="rgt")
...
def rgbFromHeight(h,r,g,b):
  ...
  for i1 in range(n1):
    for i2 in range(n2):
      ...
      b[i2][i1] = htRGB[i+2] 
      i = i+3
def horizonExtraction(k1,k2,k3,azimuth,elevation,tau,pg,png):
  f = readImage("gm")
  su = readImage("gmsu")
  sc = readImage("gmsc")
  f = gain(f)
